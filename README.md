...
Out here in the fields - where we fight for our meals - we do not have the ability to handle nulls by using Options, monads, discriminated unions or the like in a huge rewrite of billions of line of existing code. We are unsupported by existing libraries, such as Linq, which provide features we may use multiple times in a procedure, but which will return a null at the drop of a hat. 
...
If you are writing business applications, you know the call-centre cannot stop for a null value. 
The customer cannot be told to call back tomorrow. You have to code to protect from nulls, and be able to carry on when you do. You dont have the luxury to throw an exception.
...
The code here does have functions that have proved useful. But it is not a panacea - what could be after decades of Industry coding?
...
The amount of memory thrown by a null exception is enourmous
...
However, if a lot of us use the NvN() technique, then the compiler will not stand still. The DotNet compilation time will see the code in GitHub, and could easily optimise away much of the cost. (inlining the method for example, probably will already happen, or could be amde to by the team easily)

NeverNull - the name
One time I was updating the project Xml, and working out the latest incarnation of 'Nullability' and if to 'enable' or not for the solution.
...
will never be true! That is the point; NvN is a protector but does not replace other guards. (We cannot re-write the language here). To reiterate, this would be pointless:
...
You will at times have to walk down the object tree, especially when creating records. NvN() is not intended to be replacing the normal lines of code for code blocks, such as newing up the Publisher, or setting default properties. 
...
i.e if it was new, perhaps created by NvN() on the fly, then a System.Guid type
...
You wil lfind you cannot have too many of these IsValid checks.
...
In summary: with NvN() there is a slight change to conditionals: dont check for nulls, check for positive values. Lift the notion of an object existing away from being simply 'not null' to one that has at least one core property with a valid value.
 
Never Null - never return Nulls
This one is not easy!
When proving up Never Null for my use I recently finished a eight month project, coding solid for most of it, and using the Never Null approch
I thought I had done a good job. I searched for 'return null' from my methods, there were still hundred of examples.
We are so used to returning a null - it is an easy option. It has become ingrained.
Now a null return can be justified, but lets make those times special. Instead, return what the user expects.
...
public static Book BookBuilder(this Book book,Publisher publisher)
{
    ...
    //also
    if (Publisher ==null){
      //less obvious. Might depend if a Publisher is optional, from a business perspective What if it were unknown, i.e the Author is unpublished? Do I have to propagate Optional everywhere now?
    }

We have done this thousands of times before. But what if the builder looks like this, using a build concept:
...
No function can return a null, or throw a null exception. We must decide anew what it means to return early from a function, and returning an empty, but instantiated object is the best alternative, if not the only alternative, to returning a null.
...
if (string.IsNullOrEmpty(matchontitle))//cant match
  return new Book()
//call database. No match? return new Book()

}

That way, the rest of the builder chain will work. All calls should be robust, and still do the null checking, as they should assumed to be called in sequence
...
If we take this approach, we will always have an object at the end. The objection to this is that, what is the validity of the ending object? We still need to check it, and an IsValid() test is appropriate
